    ...
    def get(self, request, pk, page):
        if AuthUser(request):
            ...
            messages = message.objects.filter(
                Q(sender=request.user, receiver=pk) | Q(sender=pk, receiver=request.user)).order_by("-date_time")
            ...
            for i in c.data:
                ...
                i['user_full_name'] = Profile.objects.get(user=User.objects.get(username=i['username'])).name
                i['user_avatar'] = \
                ProfileSerializer(Profile.objects.get(user=User.objects.get(username=i['sender']))).data['profileimg']
...
class sendmessage(APIView):
    ...
    def RETURN(self, id, request):
        ...
        self.success['data']['sender_full_name'] = Profile.objects.get(
            user=User.objects.get(username=msg.data['sender'])).name
        self.success['data']['receiver_full_name'] = Profile.objects.get(
            user=User.objects.get(username=msg.data['receiver'])).name
        self.success['data']['user_full_name'] = Profile.objects.get(user=User.objects.get(username=username)).name
        self.success['data']['user_avatar'] = \
        ProfileSerializer(Profile.objects.get(user=User.objects.get(username=msg.data['sender']))).data['profileimg']
    ...
    def get(self, request):
        if request.user.is_authenticated:
            ...
            for msg in serialiser.data:
                ...
                msg['user_avatar'] = \
                ProfileSerializer(Profile.objects.get(user=User.objects.get(username=username))).data['profileimg']
